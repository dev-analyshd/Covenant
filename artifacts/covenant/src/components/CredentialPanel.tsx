...
import {
  ...
  Copy, ExternalLink, ChevronRight, Lock
} from "lucide-react";
...
const PROVING_STEPS = [
  { id: 1, label: "Hashing KYC document",           detail: "poseidon2_hash([kyc_hash, credential_secret])" },
  { id: 2, label: "Building KYC Merkle proof",      detail: "compute_merkle_root(leaf, path, indices)" },
  { id: 3, label: "Verifying sanctions clearance",  detail: "assert(sanctions_leaf ∈ NegativeScreeningTree)" },
  { id: 4, label: "Computing risk score tier",      detail: "compute_tier(risk_score) → tier ∈ {1..5}" },
  ...
  { id: 7, label: "Submitting to CovenantRegistry", detail: "register_credential(proof, public_inputs)" },
];
          ...
          <div className="space-y-5 animate-in">
            ...
            >
              ...
              <p className="text-sm" style={{ color: "#93c5fd" }}>
                Your KYC data is never stored on-chain. The Noir circuit generates a zero-knowledge proof
                — only the proof, nullifier, and compliance tier are published to the Soroban CovenantRegistry.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {["United States", "United Kingdom", "Germany", "Singapore", "Switzerland", "UAE", "Japan"].map(
            ...
            <div>
              ...
              <p className="text-xs mb-2" style={{ color: "var(--color-text-dim)" }}>
                Lower score = higher compliance tier = higher settlement limits. Computed by your compliance system.
              </p>
              <input
                type="number" min="0" max="100"
                className="input-field"
                placeholder="e.g. 15 (low risk)"
            ...
            >
              <div className="label-sm mb-2">Noir Circuit Constraints</div>
              <div className="font-mono text-xs space-y-1">
                <div style={{ color: "#475569" }}>// compliance_credential/src/main.nr</div>
                <div style={{ color: "#7dd3fc" }}>let kyc_leaf = poseidon2::hash([kyc_hash, secret]);</div>
                <div style={{ color: "#7dd3fc" }}>assert(verify_merkle_membership(kyc_leaf, root));</div>
          ...
          <div className="space-y-6 animate-in">
            <div className="text-center py-4">
              ...
              <p className="text-sm mt-1" style={{ color: "var(--color-text-muted)" }}>
                Running Noir circuit with UltraHonk proving backend
              </p>
            </div>
                ...
                return (
                  <div key={s.id} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-xs font-medium ${done ? "proof-step-done" : active ? "proof-step-active" : "proof-step-pending"}`}
                      >
                        {s.label}
                      </div>
                      {(done || active) && (
        ...
        {step === "verified" && result && (
          <div className="space-y-5 animate-in">
            <div className="text-center py-4">
              ...
              <h3 className="text-lg font-bold text-white">Credential Verified!</h3>
              <p className="text-sm mt-1" style={{ color: "var(--color-text-muted)" }}>
                ZK proof verified · CovenantRegistry updated · Nullifier stored
              </p>
            </div>
